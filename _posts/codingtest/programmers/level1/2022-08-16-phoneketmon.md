...
class Solution {
    public int solution(int[] nums) {        
        Set<Integer> set = new HashSet<Integer>(Arrays.asList(Arrays.stream(nums).boxed().toArray(Integer[]::new)));
...
주어진 배열을 set으로 중복제거 후 크기가 N/2를 넘어가지 않게 조건 적용
